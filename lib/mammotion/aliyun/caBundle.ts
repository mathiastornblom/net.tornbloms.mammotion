'use strict';

/**
 * CA bundle for the Aliyun IoT MQTT broker's TLS handshake — ported verbatim from
 * pymammotion's `pymammotion/resources/ca.pem`, which it explicitly loads via
 * `ssl.SSLContext.load_verify_locations()` instead of relying on the system/default trust
 * store (see `get_ssl_context()` in `pymammotion/transport/aliyun_mqtt.py`).
 *
 * Confirmed necessary via a real diagnostic report (2026-07-04): connecting without this
 * bundle failed with `unable to get local issuer certificate` on every attempt. The second
 * certificate here is a private "Aliyun IoT Root CA" (self-signed, CN=Aliyun IoT) that is
 * NOT a publicly cross-signed root and so isn't present in Node's bundled Mozilla CA store —
 * the GlobalSign roots alongside it likely already are, but are included anyway to match
 * pymammotion's bundle exactly rather than guess which subset is load-bearing.
 */
/** Number of individual PEM certificates bundled below — kept as a second export purely so
 *  this file isn't a single-named-export module; also a handy sanity check in tests. */
export const ALIYUN_MQTT_CA_BUNDLE_CERT_COUNT = 5;

export const ALIYUN_MQTT_CA_BUNDLE = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

UxeCp6
-----END CERTIFICATE-----
`;
